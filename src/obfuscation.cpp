    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat WPAY and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 WPAY per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeWorldPayCoinAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100WPAY+1 ( bit on if present )
    // bit 1 - 10WPAY+1
    // bit 2 - 1WPAY+1
    // bit 3 - .1WPAY+1
    // bit 3 - non-denom
...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
            if (out.nValue == Params().MasternodeCollateralClassA()) {
                if (out.scriptPubKey == payee2) return true;
            }
            if (out.nValue == Params().MasternodeCollateralClassB()) {
                if (out.scriptPubKey == payee2) return true;
            }
            if (out.nValue == Params().MasternodeCollateralClassC()) {
                if (out.scriptPubKey == payee2) return true;
            }
            if (out.nValue == Params().MasternodeCollateralClassD()) {
                if (out.scriptPubKey == payee2) return true;
            }
        }
    }
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("worldpaycoin-obfuscation");
