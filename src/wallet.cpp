...
int CWallet::GetMasternodeClass(std::string strTxHash, std::string strOutputIndex)
{
    // wait for reindex and/or import to finish
    ...
    AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_10000);
    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetMasternodeClass -- Could not locate any valid masternode vin\n");
        return -1;
    }

    if (strTxHash.empty()) // No output specified, select the first one
    {
        if (vPossibleCoins[0].tx->vout[vPossibleCoins[0].i].nValue == Params().MasternodeCollateralClassA()) { //exactly
            return CMasternode::MN_ClassA;
        }
        if (vPossibleCoins[0].tx->vout[vPossibleCoins[0].i].nValue == Params().MasternodeCollateralClassB()) { //exactly
            return CMasternode::MN_ClassB;
        }
        if (vPossibleCoins[0].tx->vout[vPossibleCoins[0].i].nValue == Params().MasternodeCollateralClassC()) { //exactly
            return CMasternode::MN_ClassC;
        }
        if (vPossibleCoins[0].tx->vout[vPossibleCoins[0].i].nValue == Params().MasternodeCollateralClassD()) { //exactly
...
CAmount CWalletTx::GetAnonymizableCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralClassA()) continue; // do not count MN-like outputs
        if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralClassB()) continue; // do not count MN-like outputs
        if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralClassC()) continue; // do not count MN-like outputs
        if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralClassD()) continue; // do not count MN-like outputs
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralClassA()) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
        if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralClassB()) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
        if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralClassC()) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
        if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralClassD()) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZWPAY = GetBoolArg("-zapwallettxes", false);
    if (fCheckZWPAY)
        zpivTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            if (fCheckZWPAY && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterNode && (pcoin->vout[i].nValue == Params().MasternodeCollateralClassA() || pcoin->vout[i].nValue == Params().MasternodeCollateralClassB() || pcoin->vout[i].nValue == Params().MasternodeCollateralClassC() || pcoin->vout[i].nValue == Params().MasternodeCollateralClassD()));
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT10000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fMasterNode) found = (pcoin->vout[i].nValue != Params().MasternodeCollateralClassA() && pcoin->vout[i].nValue != Params().MasternodeCollateralClassB() && pcoin->vout[i].nValue != Params().MasternodeCollateralClassC() && pcoin->vout[i].nValue != Params().MasternodeCollateralClassD()); // do not use Hot MN funds
                } else if (nCoinType == ONLY_10000) {
                    found = (pcoin->vout[i].nValue == Params().MasternodeCollateralClassA() || pcoin->vout[i].nValue == Params().MasternodeCollateralClassB() || pcoin->vout[i].nValue == Params().MasternodeCollateralClassC() || pcoin->vout[i].nValue == Params().MasternodeCollateralClassD());
                } else {
                    found = true;
                }
                if (!found) continue;
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-worldpaycoinstake", true)) {
        for (const COutput& out : vCoins) {
            ...
            if (fMasterNode && (out.tx->vout[out.i].nValue == Params().MasternodeCollateralClassA() || out.tx->vout[out.i].nValue == Params().MasternodeCollateralClassB() || out.tx->vout[out.i].nValue == Params().MasternodeCollateralClassC() || out.tx->vout[out.i].nValue == Params().MasternodeCollateralClassD())) {
                continue;
            }
            ...
            std::unique_ptr<CWorldPayCoinStake> input(new CWorldPayCoinStake());
    ...
    //zWPAY
    if (GetBoolArg("-zworldpaycoinstake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zWPAY set once per update interval
        bool fUpdate = false;
        ...
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZWorldPayCoinStake> input(new CZWorldPayCoinStake(meta.denom, meta.hashStake));
                listInputs.emplace_back(std::move(input));
            }
        }
    }
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 WPAY
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 WPAY over
                ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 WPAY+1 ( bit on if present )
            // bit 1 - 1000 WPAY+1
            // bit 2 - 100 WPAY+1
            // bit 3 - 10 WPAY+1
            // bit 4 - 1 WPAY+1
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (fMasterNode && (out.tx->vout[out.i].nValue == Params().MasternodeCollateralClassA() || out.tx->vout[out.i].nValue == Params().MasternodeCollateralClassB() || out.tx->vout[out.i].nValue == Params().MasternodeCollateralClassC() || out.tx->vout[out.i].nValue == Params().MasternodeCollateralClassD())) continue; //masternode input
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 WPAY collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 WPAY collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 WPAY.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 WPAY.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-worldpaycoin-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZWPAY());
            ...
            //Mark mints as spent
            if (stakeInput->IsZWPAY()) {
                CZWorldPayCoinStake* z = (CZWorldPayCoinStake*)stakeInput.get();
...
void CWallet::AutoZeromint()
{
    ...
    // zWPAY are integers > 0, so we can't mint 10% of 9 WPAY
    if (nBalance < 10) {
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zWPAY\n", nBalance);
        return;
    }

    // Percentage of zWPAY we already have
    ...
    if (dPercentage >= nZeromintPercentage) {
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zWPAY (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
            chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zWPAY amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zWPAY amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zWPAY We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough WPAY to mint this denomination
    ...
    if (nMintAmount > 0) {
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zWPAY. Current percentage of zWPAY: %lf%%\n",
...
bool CWallet::CreateZWPAYOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZWPAY(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZWPAYOutPut(denomination, outMint, dMint)) {
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to WorldPayCoin
    ...
    // Sign if these are worldpaycoin outputs - NOTE that zWPAY outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZWPAY_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZWPAY_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason, pindexCheckpoint)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZWPAY_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zWPAY privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZWPAY_INVALID_WITNESS);
            //return false;
        ...
        //Add the coin spend into a WPAY transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZWPAY_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus(_("The transaction did not verify"), ZWPAY_BAD_SERIALIZATION);
            return false;
        }

        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            //Tried to spend an already spent zWPAY
            receipt.SetStatus(_("The coin spend has been used"), ZWPAY_SPENT_USED_ZWPAY);
        ...
        receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZWPAY_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus(_("Spend Valid"), ZWPAY_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZWPAY_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zWPAY"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZWPAY_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zWPAY transaction
    ...
    if (vSelectedMints.empty()) {
        ...
        // Select the zWPAY mints to use in this spend
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZWPAY_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zWPAY require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to worldpaycoin address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZWPAY_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZWPAY_SPEND_OKAY); // Everything okay
...
void CWallet::ZWorldPayCoinBackupWallet()
{
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZWPAY_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZWPAY_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZWPAY_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZWPAY_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    receipt.SetStatus("Spend Successful", ZWPAY_SPEND_OKAY); // When we reach this point spending zWPAY was successful
