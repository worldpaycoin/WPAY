...
#if defined(NDEBUG)
#error "WPAY cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zWPAY is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zWPAY spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zWPAY without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zWPAY spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zWPAY spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zWPAY spend tx %s already in block %d",
            ...
            // Check that zWPAY mints are not already known
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    if (block.IsProofOfWork()) {
        if (!CheckProofOfWork(block.GetBlockHash(), block.nBits))
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex)
{
    ...
    if (block.GetBlockHash() != pindex->GetBlockHash()) {
        LogPrintf("%s : block=%s index=%s\n", __func__, block.GetBlockHash().ToString().c_str(), pindex->GetBlockHash().ToString().c_str());
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZWPAYStake, int nClassMN)
{
    ...
    // } else {
    //     //When zWPAY is staked, masternode only gets 2 WPAY
    //     ret = 3 * COIN;
    //     if (isZWPAYStake)
    //         ret = 2 * COIN;
    // }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from WPAY
...
{
    RenameThread("worldpaycoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZWPAYMinted()
{
...
void RecalculateZWPAYSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zWPAY supply
...
bool RecalculateWPAYSupply(int nHeightStart)
{
...
bool UpdateZWPAYSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (block.GetBlockHash() == Params().HashGenesisBlock()) {
    ...
    uint256 hashBlock = block.GetBlockHash();
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateWPAYSupply(Params().Zerocoin_StartHeight());
    }

    //Track zWPAY money supply in the block index
    if (!UpdateZWPAYSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zWPAY supply for block=%s height=%d", __func__, block.GetBlockHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zWorldPayCoinSpent: %s\n",
    ...
    if (!ValidateAccumulatorCheckpoint(block, pindex, mapAccumulators))
        return state.DoS(100, error("%s: Failed to validate accumulator checkpoint for block=%s height=%d", __func__, block.GetBlockHash().GetHex(), pindex->nHeight), REJECT_INVALID, "bad-acc-checkpoint");
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert WPAY to zWPAY
...
bool ActivateBestChain(CValidationState& state, CBlock* pblock, bool fAlreadyChecked)
{
    ...
    do {
        ...
        while (true) {
            ...
            if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetBlockHash() == pindexMostWork->GetBlockHash() ? pblock : NULL, fAlreadyChecked))
                return false;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Check timestamp
    LogPrint("debug", "%s: block=%s  is proof of stake=%d\n", __func__, block.GetBlockHash().ToString().c_str(), block.IsProofOfStake());
        ...
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            if (!tx.IsCoinBase()) {
                ...
                BOOST_FOREACH (const CTxIn& in, tx.vin) {
                    if (mapLockedInputs.count(in.prevout)) {
                        if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                            mapRejectedBlocks.insert(make_pair(block.GetBlockHash(), GetTime()));
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zWPAY spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zWPAY serial %s in block\n Block: %s",
...
{
    if (pindexPrev == NULL)
        return error("%s : null pindexPrev for block %s", __func__, block.GetBlockHash().ToString().c_str());
...
bool AcceptBlockHeader(const CBlock& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            ...
            return state.DoS(100, error("%s : prev block height=%d hash=%s is invalid, unable to add block %s", __func__, pindexPrev->nHeight, block.hashPrevBlock.GetHex(), block.GetBlockHash().GetHex()),
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zWPAY stake block is less than allowed start height", __func__);

    if (CZWorldPayCoinStake* zWPAY = dynamic_cast<CZWorldPayCoinStake*>(stake)) {
        CBlockIndex* pindexFrom = zWPAY->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zWPAY stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zWPAY stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zWPAY->GetValue()));
        if (nChecksum200 != zWPAY->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zWPAY->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.GetBlockHash() != Params().HashGenesisBlock()) {
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            ...
            return state.DoS(100, error("%s : prev block %s is invalid, unable to add block %s", __func__, block.hashPrevBlock.GetHex(), block.GetBlockHash().GetHex()),
                REJECT_INVALID, "bad-prevblk");
        }
    }

    if (block.GetBlockHash() != Params().HashGenesisBlock() && !CheckWork(block, pindexPrev))
        return false;
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZWPAY() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zWPAY fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zWPAY mints and %d zWPAY spends\n", __func__, nMints, nSpends);

    if (!CheckBlockSignature(*pblock))
        return error("ProcessNewBlock() : bad proof-of-stake block signature");

    if (pblock->GetBlockHash() != Params().HashGenesisBlock() && pfrom != NULL) {
...

    {
        ...
        if (!checked) {
            return error("%s : CheckBlock FAILED for block %s", __func__, pblock->GetBlockHash().GetHex());
        }
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(block, it->second)) {
                            ...
                            if (ProcessNewBlock(dummy, NULL, &block, &it->second)) {
                                nLoaded++;
                                queue.push_back(block.GetBlockHash());
                            }
                        }
                        range.first++;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // WPAY: We use certain sporks during IBD, so check to see if they are
        ...
        BOOST_FOREACH (const CBlockHeader& header, headers) {
            ...
            if (!AcceptBlockHeader((CBlock)header, state, &pindexLast)) {
                ...
                if (state.IsInvalid(nDoS)) {
                    if (nDoS > 0)
                        Misbehaving(pfrom->GetId(), nDoS);
                    std::string strError = "invalid header received " + header.GetBlockHash().ToString();
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            ...
            CValidationState state;
            if (!mapBlockIndex.count(block.GetBlockHash())) {
