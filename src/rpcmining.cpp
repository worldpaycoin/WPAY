...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or worldpaycoin.conf setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    if (fGenerate && Params().MineBlocksOnDemand()) {
        ...
        while (nHeight < nHeightEnd) {
            ...
            while (!CheckProofOfWork(pblock->GetBlockHash(), pblock->nBits)) {
            ...
            ++nHeight;
            blockHashes.push_back(pblock->GetBlockHash().GetHex());
        }
        return blockHashes;
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "WPAY is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "WPAY is downloading blocks...");
...
UniValue submitblock(const UniValue& params, bool fHelp)
{
    ...
    CValidationState state;
    submitblock_StateCatcher sc(block.GetBlockHash());
