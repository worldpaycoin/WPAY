...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("worldpaycoin-wallet");
...
bool CWalletDB::WriteZWPAYSeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZWPAYSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZWPAYSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZWPAYSeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZWPAYSeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZWPAYSeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZWPAYCount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZWPAYCount(uint32_t& nCount)
{
