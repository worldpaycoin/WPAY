...
#ifndef WPAY_ZWPAYWALLET_H
#define WPAY_ZWPAYWALLET_H
...
public:
    CzWPAYWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZWPAY(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZWPAY(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //WPAY_ZWPAYWALLET_H
