...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::WPAY, nValue);
    ...
    std::string BlockContentCells[] =
        {
            ...
            _("Hash"), "<pre>" + block.GetBlockHash().GetHex() + "</pre>",
...
std::string AddressToString(const CBitcoinAddress& Address)
{
    ...
    else
    {
        ...
        BOOST_FOREACH (const CDiskTxPos& pos, Txs)
        {
            CTransaction tx;
            CBlock block;
            uint256 bhash = block.GetBlockHash();
            GetTransaction(pos.nTxOffset, tx, bhash);
            std::map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.find(block.GetBlockHash());
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (worldpaycoin.conf).");
            QMessageBox::warning(this, "WPAY Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
