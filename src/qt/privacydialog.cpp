                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zWPAY ought to be enough for anybody." - Bill Gates, 2017
    ui->zWPAYpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzWPAYSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zWPAYpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzWPAY_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zWPAY is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zWPAY, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zWPAY...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zWPAY in ") +
...
void PrivacyDialog::on_pushButtonSpendzWPAY_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zWPAY is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zWPAY, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zWPAY
        sendzWPAY();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zWPAY
    sendzWPAY();
}

void PrivacyDialog::on_pushButtonZWorldPayCoinControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZWorldPayCoinControlDialog* zWorldPayCoinControl = new ZWorldPayCoinControlDialog(this);
    zWorldPayCoinControl->setModel(walletModel);
    zWorldPayCoinControl->exec();
}

void PrivacyDialog::setZWorldPayCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzWorldPayCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzWPAY()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid WorldPayCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zWPAYpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zWPAY is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " WPAY </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zWPAY</b>";
    ...
    // use mints from zWPAY selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZWorldPayCoinControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZWorldPayCoinControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zWPAY require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zWPAY"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZWPAY_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zWPAY require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zWPAY"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zWPAY was spent successfully update the addressbook with the label
    ...
    ZWorldPayCoinControlDialog::setSelectedMints.clear();
    ui->labelzWorldPayCoinSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zWPAY Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " WorldPayCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zWPAY Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zWPAY </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zWPAY "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zWPAY "));
    ui->labelzWPAYAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zWPAY </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zWPAY </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzWPAYSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzWPAY->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zWPAY
        ui->pushButtonMintzWPAY->setEnabled(false);
        ui->pushButtonMintzWPAY->setToolTip(tr("zWPAY is currently disabled due to maintenance."));

        // Spend zWPAY
        ui->pushButtonSpendzWPAY->setEnabled(false);
        ui->pushButtonSpendzWPAY->setToolTip(tr("zWPAY is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zWPAY
        ui->pushButtonMintzWPAY->setEnabled(true);
        ui->pushButtonMintzWPAY->setToolTip(tr("PrivacyDialog", "Enter an amount of WPAY to convert to zWPAY", 0));

        // Spend zWPAY
        ui->pushButtonSpendzWPAY->setEnabled(true);
        ui->pushButtonSpendzWPAY->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
