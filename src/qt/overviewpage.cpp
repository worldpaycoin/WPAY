...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::WPAY)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sWPAYPercentage, QString& szWPAYPercentage)
{
    int nPrecision = 2;
    ...
    szWPAYPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sWPAYPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
}
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& zerocoinBalance, const CAmount& unconfirmedZerocoinBalance, const CAmount& immatureZerocoinBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount nTotalBalance = balance + immatureBalance + unconfirmedBalance + nLockedBalance;
    ...
    // Percentage labels
    ui->labelWPAYPercent->setText(sPercentage);
    ui->labelzWPAYPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zWPAY.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in worldpaycoin.conf.");
    } else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in worldpaycoin.conf");
    }
    ...
    bool showWPAYAvailable = settingShowAllBalances || pivAvailableBalance != nTotalBalance;
    bool showWatchOnlyWPAYAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showWPAYPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyWPAYPending = watchUnconfBalance != 0;
    bool showWPAYLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyWPAYLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showWPAYAvailable || showWatchOnlyWPAYAvailable);
    ui->labelBalanceText->setVisible(showWPAYAvailable || showWatchOnlyWPAYAvailable);
    ui->labelWatchAvailable->setVisible(showWPAYAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showWPAYPending || showWatchOnlyWPAYPending);
    ui->labelPendingText->setVisible(showWPAYPending || showWatchOnlyWPAYPending);
    ui->labelWatchPending->setVisible(showWPAYPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showWPAYLocked || showWatchOnlyWPAYLocked);
    ui->labelLockedBalanceText->setVisible(showWPAYLocked || showWatchOnlyWPAYLocked);
    ui->labelWatchLocked->setVisible(showWPAYLocked && showWatchOnly);
    ...
    bool showzWPAYAvailable = false;                                   //settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzWPAYnconfirmed = false;                                  //settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzWPAYImmature = false;                                    //settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzWPAYAvailable);
    ui->labelzBalanceMatureText->setVisible(showzWPAYAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzWPAYnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzWPAYnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzWPAYImmature);
    ui->labelzBalanceImmatureText->setVisible(showzWPAYImmature);
    bool showPercentages = false; //! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelWPAYPercent->setVisible(showPercentages);
    ui->labelzWPAYPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("WPAY")
    updateDisplayUnit();
}
