...
std::set<std::string> ZWorldPayCoinControlDialog::setSelectedMints;
std::set<CMintMeta> ZWorldPayCoinControlDialog::setMints;

ZWorldPayCoinControlDialog::ZWorldPayCoinControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZWorldPayCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZWorldPayCoinControlDialog::~ZWorldPayCoinControlDialog()
{
    delete ui;
}

void ZWorldPayCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWorldPayCoinControlDialog::updateList()
{
...
void ZWorldPayCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWorldPayCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZWorldPayCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWorldPayCoinControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZWorldPayCoinControlDialog::GetSelectedMints()
{
...
void ZWorldPayCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
