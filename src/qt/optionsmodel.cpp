...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::WPAY);
    ...
    if (!settings.contains("nAnonymizeWorldPayCoinAmount"))
        settings.setValue("nAnonymizeWorldPayCoinAmount", 1000);

    nAnonymizeWorldPayCoinAmount = settings.value("nAnonymizeWorldPayCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeWorldPayCoinAmount"))
        SoftSetArg("-anonymizeworldpaycoinamount", settings.value("nAnonymizeWorldPayCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in worldpaycoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeWorldPayCoinAmount:
            return QVariant(nAnonymizeWorldPayCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeWorldPayCoinAmount:
            nAnonymizeWorldPayCoinAmount = value.toInt();
            settings.setValue("nAnonymizeWorldPayCoinAmount", nAnonymizeWorldPayCoinAmount);
            emit anonymizeWorldPayCoinAmountChanged(nAnonymizeWorldPayCoinAmount);
            break;
        case CoinControlFeatures:
