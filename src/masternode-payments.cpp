...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZWPAYStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev)
        return;
    ...
    // } else {
    masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZWPAYStake);
    // }
}
    ...
    bool fZWPAYStake)
{
    ...
    CScript payee;
    int nClassMN = CMasternode::MN_ClassA;
    ...
    std::vector<pair<int, CMasternode> > vMasternodeRanks = mnodeman.GetMasternodeRanks(pindexPrev->nHeight);
    BOOST_FOREACH (PAIRTYPE(int, CMasternode) & s, vMasternodeRanks) {
        CMasternode* mn = mnodeman.Find(s.second.vin);
        if (mn != NULL) {
            CScript tmpPayee;
            tmpPayee = GetScriptForDestination(
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight,
        blockValue, 0, fZWPAYStake, nClassMN);
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount requiredMasternodePayment = GetMasternodePayment(nBlockHeight,
        nReward, nMasternode_Drift_Count, txNew.IsZerocoinSpend(), 0);
