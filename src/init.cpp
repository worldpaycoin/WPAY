...
#include "config/worldpaycoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("worldpaycoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "worldpaycoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "worldpaycoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the WPAY and zWPAY money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 39999, 38999));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in WPAY/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in WPAY/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, worldpaycoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in WPAY/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all WPAY specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-worldpaycoinstake=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-worldpaycoinstake=<n>", strprintf(_("Enable or disable staking functionality for WPAY inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zworldpaycoinstake=<n>", strprintf(_("Enable or disable staking functionality for zWPAY inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:39999"));
    ...
    strUsage += HelpMessageOpt("-backupzpiv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zWPAY minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpivbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zWPAY backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeworldpaycoinamount=<n>     " + strprintf(_("Keep N WPAY anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 39998, 38998));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The ZillionFLUX Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The WPAY Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("worldpaycoin-loadblk");
...
/** Sanity checks
 *  Ensure that WPAY is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (fDisableWallet) {
#endif
        if (SoftSetBoolArg("-worldpaycoinstake", false))
            LogPrintf("AppInit2 : parameter interaction: wallet functionality not enabled -> setting -worldpaycoinstake=0\n");
#ifdef ENABLE_WALLET
    }
#endif
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. WPAY Core is shutting down."));
...
#endif
    // Make sure only a single WPAY process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. WPAY Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //WPAY: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //WPAY specific: zerocoin and spork DB's
                ...
                // WPAY: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZWPAYMinted();
                        RecalculateZWPAYSpent();
                    }
                    RecalculateWPAYSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // WPAY: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of WPAY Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart WPAY Core to complete") << "\n";
        ...
        zwalletMain = new CzWPAYWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zWPAYWallet
        uiInterface.InitMessage(_("Syncing zWPAY wallet..."));

        bool fEnableZWorldPayCoinBackups = GetBoolArg("-backupzpiv", true);
        pwalletMain->setZWorldPayCoinAutoBackups(fEnableZWorldPayCoinBackups);
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeWorldPayCoinAmount = 2;
...
//    }
//
//    nAnonymizeWorldPayCoinAmount = GetArg("-anonymizeworldpaycoinamount", 0);
//    if (nAnonymizeWorldPayCoinAmount > 999999) nAnonymizeWorldPayCoinAmount = 999999;
//    if (nAnonymizeWorldPayCoinAmount < 2) nAnonymizeWorldPayCoinAmount = 2;
    ...
    LogPrintf("Anonymize WPAY Amount %d\n", nAnonymizeWorldPayCoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1WPAY+1000 == (.1WPAY+100)*10
       10WPAY+10000 == (1WPAY+1000)*10
    */
