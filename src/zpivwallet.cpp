...
CzWPAYWallet::CzWPAYWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZWPAYSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZWPAYSeed_deprecated()) {
                    LogPrintf("%s: Updated zWPAY seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzWPAYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZWPAYCount(nCountLastUsed);
    else if (!walletdb.ReadZWPAYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzWPAYWallet::Lock()
{
    seedMaster = 0;
}

void CzWPAYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzWPAYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZWPAY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzWPAYWallet::LoadMintPoolFromDB()
{
...
void CzWPAYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzWPAYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzWPAYWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzWPAYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZWPAY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzWPAYWallet::SeedToZWPAY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzWPAYWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzWPAYWallet::GenerateDeterministicZWPAY(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzWPAYWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzWPAYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
