...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 WPAY at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zWPAY send
enum ZerocoinSpendStatus {
    ZWPAY_SPEND_OKAY = 0,                            // No error
    ZWPAY_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZWPAY_WALLET_LOCKED = 2,                         // Wallet was locked
    ZWPAY_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZWPAY_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZWPAY_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZWPAY_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZWPAY_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZWPAY_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZWPAY_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZWPAY_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZWPAY_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZWPAY_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZWPAY_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZWPAY_SPENT_USED_ZWPAY = 14,                      // Coin has already been spend
    ZWPAY_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZWPAY_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZWorldPayCoinBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZWPAYOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzWPAYWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzWPAYTracker> zpivTracker;
    ...
    void setZWallet(CzWPAYWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzWPAYTracker>(new CzWPAYTracker(strWalletFile));
    }

    CzWPAYWallet* getZWallet() { return zwalletMain; }
    ...
    void setZWorldPayCoinAutoBackups(bool fEnabled)
    {
    ...
    /// Get class which can be used for the Masternode
    int GetMasternodeClass(std::string strTxHash = "", std::string strOutputIndex = "");
    ...
    /** zWPAY reset */
    boost::signals2::signal<void()> NotifyzWPAYReset;
