...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    // zWPAY spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zWPAY spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zWPAY serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (nHeight > 10) {
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                //For the period before v2 activation, zWPAY will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("WPAYMiner : generated block is stale");
    }
    ...
    // Track how many getdata requests this block gets
    {
        ...
        wallet.mapRequestCount[pblock->GetBlockHash()] = 0;
    }

    // Inform about the new block
    GetMainSignals().BlockFound(pblock->GetBlockHash());
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("WPAYMiner : ProcessNewBlock, block not accepted");
    }

    for (CNode* node : vNodes) {
        node->PushInventory(CInv(MSG_BLOCK, pblock->GetBlockHash()));
    }
...
{
    LogPrintf("WPAYMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("worldpaycoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //Stake miner main
        if (fProofOfStake) {
            LogPrintf("CPUMiner : proof-of-stake block found %s \n", pblock->GetBlockHash().ToString().c_str());
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zWPAY with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zWPAY key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("BitcoinMiner(): Signing new block with zWPAY key failed \n");
                    continue;
                }
        ...
        LogPrintf("Running WPAYMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
